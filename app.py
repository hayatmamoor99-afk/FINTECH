...
st.header("💰 Future Investment Simulator")
...
c4.metric(
    "Probability Gain",
    f"{probability_gain:.1f}%"
)

sim_fig = px.histogram(
    investment_results,
    nbins=50,
    title="Future Investment Value Distribution"
)
...
This stock currently exhibits
{trend.lower()} characteristics.

Historical analysis suggests
a {probability_up:.1f}% probability
of positive daily returns.

The stock carries a
{risk_level.lower()} level of risk.

Monte Carlo simulations indicate
an expected investment value of
${expected_value:.2f} over the selected horizon.
